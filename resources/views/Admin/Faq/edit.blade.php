...
<form class="form-validate-jquery" enctype="multipart/form-data" method="post"
      action="{{route('faqs.update',['faq'=>$faq->id]) }}">
                            ...
                            <div id="basic-justified-tab{{$key}}"
                                    ...
                                    <div class="col-lg-10">
                                        <div class="input-group">
                                            <input type="text" name="localization[{{$language->id}}][title]" required class="form-control"
                                                   value="{{ $faq->localizations->where('language_id',$language->id)->first() ? $faq->localizations->where('language_id',$language->id)->first()->title : "" }}"
                                ...
                                <div class="mb-3">
                                    <textarea rows="5" cols="5" name="localization[{{$language->id}}][text]" id="text" class="editor" required
                                              placeholder="">{!!  $faq->localizations->where('language_id',$language->id)->first() ? $faq->localizations->where('language_id',$language->id)->first()->text : "" !!}</textarea>
                                </div>
                            </div>
                        @endforeach
                    </div>
